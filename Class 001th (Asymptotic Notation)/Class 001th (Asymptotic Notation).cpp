...
#pragma region 시간 복잡도
	...
	// 표시는 빅-오 (big-O) 노테이션(표기법)을 사용합니다.
	// 표기법은 빅오, 빅세타, 빅오메가 표기법이 있는데, 
	// 빅오는 알고리즘 효율성의 상향선을 기준, 빅오메가는 하향선을 기준,
	// 빅세타는 그 사이를 기준으로 한다.
...
#pragma region O(n) 선형 시간
	// 입력에 따라 걸리는 시간이 선형적으로 증가되는 시간복잡도입니다.
...
#pragma region O(n-log n) 로그 선형 시간
	// 지수 k가 로그 항의 하나와 동일한 유사 선형 시간을 
	// 나타내는 형태의 시간복잡도입니다.

	// 입력 크기 n에 대해 선형적으로 증가하지만, 로그 항이 곱해지는 시간 복잡도입니다.

	// ex) 정렬 알고리즘
	// 비교하기(n) + 내부적 분할작업(log n)
#pragma endregion
